...
import { normalize as posixNormalize } from "../../path/1.1.1/posix/normalize.js";
import { extname } from "../../path/1.1.1/extname.js";
import { join } from "../../path/1.1.1/join.js";
import { relative } from "../../path/1.1.1/relative.js";
import { resolve } from "../../path/1.1.1/resolve.js";
import { SEPARATOR_PATTERN } from "../../path/1.1.1/constants.js";
import { exists } from "../../fs/1.0.19/exists.js";
import { contentType } from "../../media-types/1.1.0/content_type.js";
...
import { ByteSliceStream } from "../../streams/1.0.10/byte_slice_stream.js";
import { parseArgs } from "../../cli/1.0.21/parse_args.js";
import denoConfig from "./deno.js";
import { format as formatBytes } from "../../fmt/1.0.8/bytes.js";
import { getNetworkAddress } from "../../net/1.0.4/unstable_get_network_address.js";
import { escape } from "../../html/1.0.4/entities.js";
...
const ENV_PERM_STATUS =
  dntShim.Deno.permissions.querySync?.({ name: "env", variable: "DENO_DEPLOYMENT_ID" })
    .state ?? "granted"; // for deno deploy
const NET_PERM_STATUS =
  dntShim.Deno.permissions.querySync?.({ name: "sys", kind: "networkInterfaces" })
    .state ?? "granted"; // for deno deploy
const DENO_DEPLOYMENT_ID = ENV_PERM_STATUS === "granted"
  ? dntShim.Deno.env.get("DENO_DEPLOYMENT_ID")
  : undefined;
...
export async function serveFile(
  ...
  try {
    fileInfo ??= await dntShim.Deno.stat(filePath);
  } catch (error) {
  ...
  if (rangeValue && 0 < fileSize) {
    ...
    const file = await dntShim.Deno.open(filePath);
    await file.seek(start, dntShim.Deno.SeekMode.Start);
...
async function serveDirIndex(
  ...
  if (dirUrl !== "/") {
    ...
    const entryInfo = dntShim.Deno.stat(prevPath).then((fileInfo): EntryInfo => ({
  ...
  for await (const entry of dntShim.Deno.readDir(dirPath)) {
    ...
    listEntryPromise.push((async () => {
      try {
        const fileInfo = await dntShim.Deno.stat(filePath);
        return {
...
export async function serveDir(
  ...
  try {
    ...
    response = error instanceof dntShim.Deno.errors.NotFound
...
async function createServeDirResponse(
  ...
  const fileInfo = await dntShim.Deno.stat(fsPath);
  ...
  if (showIndex) { // serve index.html
    ...
    let indexFileInfo: dntShim.Deno.FileInfo | undefined;
    try {
      indexFileInfo = await dntShim.Deno.lstat(indexPath);
    } catch (error) {
      if (!(error instanceof dntShim.Deno.errors.NotFound)) {
        throw error;
      }
...
function main() {
  const serverArgs = parseArgs(dntShim.Deno.args, {
  ...
  function onListen({ port, hostname }: { port: number; hostname: string }) {
    ...
    const host = (dntShim.Deno.build.os === "windows" && hostname === "0.0.0.0")
      ? "localhost"
      : hostname;
  ...
  const options: {
    port?: number;
    hostname?: string;
    onListen?: (localAddr: dntShim.Deno.NetAddr) => void;
    cert?: string;
    key?: string;
  } = {
    hostname: host,
  ...
  if (useTls) {
    options.cert = dntShim.Deno.readTextFileSync(certFile);
    options.key = dntShim.Deno.readTextFileSync(keyFile);
  }
  dntShim.Deno.serve(options, handler);
}
